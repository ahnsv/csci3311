...
# Main content
st.markdown("""
This interactive tool allows you to explore detailed data on college affordability, costs, debt, and outcomes.
Select a tool from the options below to begin your analysis.
""")
...
tool_options = {
    "data_explorer": "Data Explorer - Filter and analyze college data by year, state, and institution type",
    "tuition_trends": "Tuition Trends - Visualize tuition trends over time with interactive charts",
    "debt_analysis": "Debt Analysis - Explore student debt patterns across different demographics",
    "roi_calculator": "ROI Calculator - Calculate and compare return on investment for different institutions"
}
    ...
    for year, schools in data_by_year.items():
        for school in schools:
            ...
            school_data = {
                ...
                "median_debt": pd.to_numeric(school.get(f"{year}.aid.median_debt.completers.overall"), errors="coerce"),
                "median_debt_low_income": pd.to_numeric(school.get(f"{year}.aid.median_debt.income.0_30000"), errors="coerce"),
                "median_debt_mid_income": pd.to_numeric(school.get(f"{year}.aid.median_debt.income.30001_75000"), errors="coerce"),
                "median_debt_high_income": pd.to_numeric(school.get(f"{year}.aid.median_debt.income.greater_than_75000"), errors="coerce"),
                "earnings_6yr": pd.to_numeric(school.get(f"{year}.earnings.6_yrs_after_entry.median"), errors="coerce"),
                "earnings_7yr": pd.to_numeric(school.get(f"{year}.earnings.7_yrs_after_entry.median"), errors="coerce"),
                "earnings_8yr": pd.to_numeric(school.get(f"{year}.earnings.8_yrs_after_entry.median"), errors="coerce"),
                "enrollment": pd.to_numeric(school.get(f"{year}.student.size"), errors="coerce")
            }
            ...
            if school_data["median_debt"] > 0 and school_data["earnings_6yr"] > 0:
                ...
                school_data["roi"] = estimated_10yr_earnings - school_data["median_debt"]
            else:
    ...
    if st.button("Load Data"):
        with st.spinner("Fetching data from College Scorecard API..."):
            try:
                ...
                table_year = st.selectbox("Select Year for Detailed Data", selected_years, index=len(selected_years)-1)
                ...
                display_cols = [
                    "name", "state", "ownership", "tuition_in_state", 
                    "tuition_out_of_state", "median_debt", "earnings_6yr", "roi"
                ]
                ...
                display_df.columns = [
                    "Institution", "State", "Type", "In-State Tuition ($)", 
                    "Out-of-State Tuition ($)", "Median Debt ($)", 
                    "Earnings - 6yr ($)", "ROI - 10yr Estimate ($)"
                ]
                
                # Format currency columns
                currency_cols = [
                    "In-State Tuition ($)", "Out-of-State Tuition ($)", 
                    "Median Debt ($)", "Earnings - 6yr ($)", "ROI - 10yr Estimate ($)"
                ]
        ...
        st.info("Select your filters and click 'Load Data' to explore college affordability metrics.")
        
        # Check if we have data from a previous load
        if hasattr(st.session_state, 'college_data'):
            st.success("Using previously loaded data. Adjust filters and click 'Load Data' to refresh.")
